...
export class CDatoCotizacion implements OnInit, OnDestroy {
    ...
    get formCot() {
        return this.frmDatosCot.controls;
    }
    get formProveedor() {
    ...
    grabarCotizacion() {
        ...
        const $procesarCotizacion = this.serviceOportunidad.procesarCotizacion(cotizacion).subscribe({
            next: (rpta: any) => {
    ...
    listarCotizacionUno() {
        ...
        const $listarCotizacionUno = this.serviceOportunidad.listarCotizacionUno(this.idCotiza).subscribe({
            next: (rpta: any) => {
                this.setSpinner(false);
        ...
        if (refItem) {
            refItem.onClose.subscribe((rpta: any) => {
                if (rpta != undefined) {
                    ...
                    this.lstCotizacionItem.update((items) => {
                        const newItems = [...items];
                        const _posAll = newItems.findIndex((x) => x.nroindex == index);
                        console.log('_posAll : ', _posAll);
                        if (_posAll != -1) newItems.splice(_posAll, 1);
                        newItems.push(rpta.data);
                        return newItems;
                    });
                }
                this.calcularTotales();
    ...
    async eliminarItem(data: any) {
        ...
        this.lstCotizacionItem.update((items) => {
            const newItems = [...items];
            const _posAll = data.idcotizaitem > 0
                ? newItems.findIndex((x) => x.idcotizaitem == data.idcotizaitem)
                : newItems.findIndex((x) => x.idnvoitem == data.idnvoitem);
            if (_posAll != -1) newItems.splice(_posAll, 1);
            return newItems;
        });
    }
    ...
    guardarProveedor() {
        ...
        if (this.submitted()) {
            this.kanbanService.prcClientes(this.registerFormCliente.getRawValue()).subscribe({
                next: (rpta: any) => {
    ...
    guardarContacto() {
        ...
        if (this.submitted()) {
            ...
            this.asignadosContacto().unshift(
                (this.contacto = {
